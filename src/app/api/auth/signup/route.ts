/**
 * /api/auth/signup — create a new user account.
 *
 * Two bug fixes from v1:
 *   1. New users now inherit the caller's tenantId so they appear in the
 *      tenant employees list. Previously tenantId was null → ghost users.
 *   2. Validation errors now surface the failing field so "Invalid input"
 *      isn't a black box. Duplicate emails return 409 with a clear message
 *      whether they're caught by the pre-check or by Prisma's unique constraint.
 *
 * Bootstrap path (very first user, no caller, no tenant) is unchanged —
 * still creates an ADMIN with tenantId=null. That user can then create a
 * tenant via /superadmin and re-sign with the proper tenant context.
 */

import { NextResponse } from "next/server";
import bcrypt from "bcryptjs";
import { z } from "zod";
import { prisma } from "@/lib/db";
import { getServerAuth } from "@/lib/auth";
import { getScopedLocationIds } from "@/lib/guards";

const schema = z.object({
  email: z.string().email("Email must be a valid address"),
  name: z.string().min(1, "Name is required"),
  password: z.string().min(6, "Password must be at least 6 characters"),
  role: z.enum(["ADMIN", "MANAGER", "LEAD", "EMPLOYEE"]).optional(),
  department: z.string().nullable().optional(),
  hourlyWage: z.number().nonnegative().optional(),
  isTipped: z.boolean().optional(),
  locationIds: z.array(z.string()).optional(),
  // Profile fields (all optional, all nullable)
  photoUrl: z.string().nullable().optional(),
  phone: z.string().nullable().optional(),
  address: z.string().nullable().optional(),
  dateOfBirth: z.string().nullable().optional(),
  hireDate: z.string().nullable().optional(),
  employmentType: z.enum(["W2", "CONTRACTOR_1099", "UNSPECIFIED"]).optional(),
  emergencyContactName: z.string().nullable().optional(),
  emergencyContactPhone: z.string().nullable().optional(),
  emergencyContactRelation: z.string().nullable().optional(),
  notes: z.string().nullable().optional(),
});

export async function POST(req: Request) {
  const body = await req.json().catch(() => null);
  const parsed = schema.safeParse(body);
  if (!parsed.success) {
    // Surface the first failing field so users know what to fix.
    const first = parsed.error.issues[0];
    const field = first.path.length > 0 ? first.path.join(".") : "input";
    return NextResponse.json(
      { error: `Invalid input on ${field}: ${first.message}`, issues: parsed.error.issues },
      { status: 400 },
    );
  }

  const userCount = await prisma.user.count();
  const session = await getServerAuth();
  const callerRole = (session?.user as any)?.role as
    | "ADMIN"
    | "MANAGER"
    | "LEAD"
    | "EMPLOYEE"
    | undefined;
  const callerId = (session?.user as any)?.id as string | undefined;
  const callerTenantId = (session?.user as any)?.tenantId as string | null | undefined;

  // Bootstrap: first signup with no users yet creates an ADMIN with no tenant.
  // That admin can then create a tenant via /superadmin and onboard from there.
  const isBootstrap = userCount === 0;

  if (!isBootstrap) {
    if (callerRole !== "ADMIN" && callerRole !== "MANAGER") {
      return NextResponse.json(
        { error: "Only admins or managers can create accounts" },
        { status: 403 },
      );
    }
    if (!callerTenantId) {
      return NextResponse.json(
        { error: "Your account is not attached to a tenant. Ask a super-admin." },
        { status: 400 },
      );
    }
  }

  const {
    email,
    name,
    password,
    role: requestedRole,
    department,
    hourlyWage,
    isTipped,
    locationIds,
  } = parsed.data;

  // Determine the actual role to assign
  let assignedRole: "ADMIN" | "MANAGER" | "LEAD" | "EMPLOYEE";
  if (isBootstrap) {
    assignedRole = "ADMIN";
  } else if (callerRole === "ADMIN") {
    assignedRole = requestedRole ?? "EMPLOYEE";
  } else {
    // MANAGER: can only create EMPLOYEE or LEAD
    if (requestedRole && requestedRole !== "EMPLOYEE" && requestedRole !== "LEAD") {
      return NextResponse.json(
        { error: "Managers can only create Employees or Leads" },
        { status: 403 },
      );
    }
    assignedRole = requestedRole ?? "EMPLOYEE";
  }

  // Determine final location assignments (managers must stay in their lane)
  let assignedLocationIds: string[] = locationIds ?? [];
  if (!isBootstrap && callerRole === "MANAGER" && callerId) {
    const managerLocs = await getScopedLocationIds(callerId, "MANAGER");
    if (!managerLocs || managerLocs.length === 0) {
      return NextResponse.json(
        { error: "You are not assigned to any location, so you cannot add staff." },
        { status: 403 },
      );
    }
    if (assignedLocationIds.length === 0) {
      assignedLocationIds = managerLocs;
    } else {
      const allowed = new Set(managerLocs);
      for (const id of assignedLocationIds) {
        if (!allowed.has(id)) {
          return NextResponse.json(
            { error: "You can only assign new staff to your own location(s)." },
            { status: 403 },
          );
        }
      }
    }
  }

  // Pre-flight duplicate-email check (cheaper than catching Prisma's
  // P2002 + clearer error message).
  const existing = await prisma.user.findUnique({
    where: { email: email.toLowerCase() },
    select: { id: true },
  });
  if (existing) {
    return NextResponse.json(
      { error: "Email already in use" },
      { status: 409 },
    );
  }

  const {
    photoUrl,
    phone,
    address,
    dateOfBirth,
    hireDate,
    employmentType,
    emergencyContactName,
    emergencyContactPhone,
    emergencyContactRelation,
    notes,
  } = parsed.data;

  // ---- THE BUG FIX ----
  // Every non-bootstrap user MUST inherit the caller's tenantId so they
  // appear in tenant-scoped queries (employees list, payroll, schedule, etc.).
  try {
    const user = await prisma.user.create({
      data: {
        email: email.toLowerCase(),
        name,
        passwordHash: await bcrypt.hash(password, 10),
        role: assignedRole,
        tenantId: isBootstrap ? null : callerTenantId!,
        department: department || null,
        hourlyWage: hourlyWage ?? 0,
        isTipped: isTipped ?? false,
        photoUrl: photoUrl || null,
        phone: phone || null,
        address: address || null,
        dateOfBirth: dateOfBirth ? new Date(dateOfBirth) : null,
        hireDate: hireDate ? new Date(hireDate) : null,
        employmentType: employmentType ?? "UNSPECIFIED",
        emergencyContactName: emergencyContactName || null,
        emergencyContactPhone: emergencyContactPhone || null,
        emergencyContactRelation: emergencyContactRelation || null,
        // Notes is admin-only — managers can't set this on creation
        notes: callerRole === "ADMIN" ? (notes || null) : null,
      },
      select: { id: true, email: true, name: true, role: true, tenantId: true },
    });

    if (assignedLocationIds.length > 0) {
      await prisma.employeeLocation.createMany({
        data: assignedLocationIds.map((locationId) => ({
          userId: user.id,
          locationId,
        })),
        skipDuplicates: true,
      });
    }

    return NextResponse.json({ user });
  } catch (e: any) {
    // Prisma P2002 = unique constraint (race with the pre-check above)
    if (e?.code === "P2002") {
      return NextResponse.json(
        { error: "Email already in use" },
        { status: 409 },
      );
    }
    console.error("signup failed:", e);
    return NextResponse.json(
      { error: `Signup failed: ${e?.message ?? "unknown error"}` },
      { status: 500 },
    );
  }
}
