/**
 * v12.1: TENANT-SCOPED timesheets API.
 */
...
export async function GET(req: Request) {
  ...
  const prisma = scopedPrisma(tenantId);
  ...
  if (formatType === "csv") {
    ...
    const headers = ["Employee", "Email", "Department", "Clock In", "Clock Out", "Hours",
      ...(includeCost ? ["Wage", "Pay"] : []), "Edited"];
    const rows = [headers.join(",")];
    let totalH = 0; let totalP = 0;
    const sorted = [...viewable].sort((a, b) => new Date(a.clockIn).getTime() - new Date(b.clockIn).getTime());
    for (const e of sorted) {
      const h = e.clockOut ? durationHours(e.clockIn, e.clockOut) : 0;
      const pay = h * (e.user.hourlyWage ?? 0);
      totalH += h; totalP += pay;
      rows.push([
        JSON.stringify(e.user.name), JSON.stringify(e.user.email), JSON.stringify(e.user.department ?? ""),
    ...
    rows.push(["TOTAL", "", "", "", "", totalH.toFixed(2), ...(includeCost ? ["", totalP.toFixed(2)] : []), ""].join(","));
    return new Response(rows.join("\n"), {
      headers: {
        "Content-Type": "text/csv",
        "Content-Disposition": `attachment; filename="timesheets-${format(new Date(), "yyyy-MM-dd")}.csv"`,
      },
    });
  }
