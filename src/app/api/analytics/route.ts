/**
 * v12.1: TENANT-SCOPED analytics API.
 * Same logic as v11, but uses scopedPrisma to auto-filter every query by tenantId.
 */
...
import {
  startOfDay, endOfDay, startOfWeek, endOfWeek, subWeeks, format, addDays,
} from "date-fns";
...
export async function GET(req: Request) {
  ...
  const prisma = scopedPrisma(auth.tenantId); // CRITICAL: all queries auto-scope to tenant
  ...
  const scopedEmployeeIds = auth.role === "MANAGER"
    ? await getScopedEmployeeIds(auth.userId, "MANAGER")
    : null;
  const scopedLocationIds = auth.role === "MANAGER"
    ? await getScopedLocationIds(auth.userId, "MANAGER")
    : null;

  const locationWhere = (() => {
    if (locationFilter) {
      if (scopedLocationIds && !scopedLocationIds.includes(locationFilter)) {
        return { id: "__none__" };
      }
      return { id: locationFilter };
    }
    if (scopedLocationIds) return { id: { in: scopedLocationIds } };
    return undefined;
  })();
  ...
  const todayShiftsRaw = await prisma.shift.findMany({
    where: {
      ...
      ...(scopedLocationIds && !locationFilter ? { locationId: { in: scopedLocationIds } } : {}),
    },
  ...
  const openClockIns = empIds.length > 0
    ...
    : [];
  const recentClockOuts = empIds.length > 0
    ? await prisma.clockEntry.findMany({ where: { userId: { in: empIds }, clockOut: { gte: todayStart } } })
    : [];
  ...
  const roster = todayShiftsRaw.map((s) => {
    ...
    return {
      shiftId: s.id, employee: s.employee, location: s.location,
      startTime: s.startTime, endTime: s.endTime, role: s.role, status,
  ...
  const scheduledThisWeek = await prisma.shift.findMany({
    ...
    select: {
      startTime: true, endTime: true, employeeId: true,
      employee: { select: { hourlyWage: true } }, locationId: true,
    },
  });
  ...
  if (isAdmin) {
    const weeks: Date[] = [];
    for (let i = 7; i >= 0; i--) weeks.push(startOfWeek(subWeeks(now, i), { weekStartsOn: 1 }));
    ...
    trend = weeks.map((wStart) => {
      ...
      const inWeek = trendEntries.filter((e) => e.clockIn >= wStart && e.clockIn <= wEnd);
      const hours = inWeek.reduce((acc, e) => acc + durationHours(e.clockIn, e.clockOut), 0);
      const cost = inWeek.reduce((acc, e) =>
        acc + durationHours(e.clockIn, e.clockOut) * (e.user.hourlyWage ?? 0), 0);
      return { week: format(wStart, "MMM d"), hours: Number(hours.toFixed(1)), cost: Number(cost.toFixed(2)) };
    });
  }

  let locationComparison: Array<{ locationId: string; locationName: string; hours: number; cost: number }> = [];
  if (isAdmin) {
    ...
    const locShifts = await prisma.shift.findMany({
      where: { startTime: { gte: weekStart, lte: weekEnd }, published: true, locationId: { not: null } },
  ...
  const filterableLocations = await prisma.location.findMany({
    where: scopedLocationIds ? { active: true, id: { in: scopedLocationIds } } : { active: true },
    select: { id: true, name: true },
    orderBy: { name: "asc" },
  });
  ...
  return NextResponse.json({
    ...
    weeklyByDay, trend, locationComparison, filterableLocations,
